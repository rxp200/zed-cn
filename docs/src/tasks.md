...
- `ctrl-alt-n` runs {#action editor::RunCode}: executes selected code when present, otherwise saves and runs the current file without requiring a gutter runnable. Use {#action editor::RunFile} to always run the whole file, or {#action editor::RunSelection} to require a selection. Toolbar and context-menu controls provide run and stop. `ctrl-alt-shift-j` runs {#action editor::StopCode}; the existing workspace-sidebar shortcut is unchanged. The original {#action editor::SpawnNearestTask} command remains available.
- The built-in file runner supports Python, JavaScript, TypeScript, C# projects, Ruby, PHP, Perl, Lua, Bash, PowerShell (`pwsh`, or Windows PowerShell for file execution), R, Julia, Dart, Elixir, Haskell, OCaml, Groovy, Kotlin `.kts`, Swift, Java source launch, Go, and standalone C/C++/Rust programs. Tools must already be installed in the execution environment; no packages are downloaded automatically. Python uses the selected toolchain when available, then `.venv` and system interpreters. JavaScript/TypeScript honor file-directory/project-root Bun lockfiles and Deno configuration. Otherwise JavaScript uses Node with Bun fallback; TypeScript checks file-directory/project-root `node_modules/.bin/tsx`, then PATH. Deno uses cached-only dependencies without automatically granting permissions.
- Files run from their own directory in the existing local or remote task terminal. Runner output opens without taking focus from the editor; click the terminal when the program needs interactive input. This also applies to selection execution and runner task overrides. Compiler output uses a fresh temporary directory, is executed only after successful compilation, and is cleaned up afterward. C/C++ on Windows currently requires GCC-compatible `gcc`/`g++`; Java source launch requires a compatible JDK.
- A task tagged `run-current` overrides the built-in runner. Project tasks take precedence over global tasks; multiple matching tasks at the winning scope prompt for one choice, and the most recently scheduled matching label is reused on the next invocation. Use such tasks for project-specific arguments, interpreters, working directories, and build targets. Cargo projects use `cargo run --offline`, Go modules run the current package with dependency downloads disabled, Swift packages use `swift run` without automatic dependency resolution, and a single enclosing C# project uses `dotnet run --no-restore`. Build-tool default target selection applies; ambiguous or non-executable targets produce the tool's diagnostic rather than guessing. Maven/Gradle and CMake/Make/Meson projects still require an explicit target task. Headers, JSX/TSX, unnamed buffers, multi-file views, restricted projects, and unsupported languages are not run automatically.
- The default shortcut now runs files rather than the nearest test. To restore the previous behavior, bind `ctrl-alt-n` to `editor::SpawnNearestTask` in your user keymap. The stop action affects only runner-managed tasks in this workspace: local task processes are killed through existing terminal process-group handling; remote tasks receive a terminal interrupt, so programs that ignore interrupts may need manual termination. Running tasks are not duplicated; stop the active task before rerunning. Selection execution supports Python, JavaScript, Ruby, PHP, Perl, Lua, Bash, PowerShell, Julia, R, and Elixir, with a 16 KiB limit and no inherited file-local variables/imports. Other languages can supply a `run-selection` task. File-backed buffers are required, and the source file is saved before either execution mode.
