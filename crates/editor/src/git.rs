...
impl Editor {
    ...
    pub(super) fn blame_revision(
        ...
        let Some((blame_entry, repository)) = self.blame_entry_at_cursor(window, cx) else {
            self.show_blame_revision_toast("该行没有 blame 记录", cx);
            return;
        };
        if blame_entry.sha.is_zero() {
            self.show_blame_revision_toast("无法定位到该修订：该行尚未提交", cx);
            return;
        }
        if self
            .blame
        ...
        {
            self.show_blame_revision_toast("已在该修订上查看 blame", cx);
            return;
        }
    ...
    pub(super) fn blame_previous_revision(
        ...
        let Some((revision, filename)) = blame_entry.previous_sha_and_filename() else {
            self.show_blame_revision_toast("该行没有上一个修订", cx);
            return;
        };
