...
pub struct QuickActionBar {
    ...
    _web_preview_subscription: Option<Subscription>,
...
impl QuickActionBar {
    pub fn new(
        ...
        let web_preview_subscription =
            web_preview::preview_state(cx).map(|state| cx.observe(&state, |_, _, cx| cx.notify()));

        let mut this = Self {
            _inlay_hints_enabled_subscription: None,
            _ai_settings_subscription: ai_settings_subscription,
            _web_preview_subscription: web_preview_subscription,
            active_item: None,
            buffer_search_bar,
            show: true,
