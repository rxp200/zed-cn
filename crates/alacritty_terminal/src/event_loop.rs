    ...
    pub fn spawn(mut self) -> JoinHandle<(Self, State)> {
        thread::spawn_named("PTY reader", move || {
            ...
            'event_loop: loop {
                ...
                // A continuously readable PTY can make poll return events after the timeout
                // expires. Check the deadline itself instead of treating an empty poll result as
                // proof that the synchronized update timed out.
                if sync_timeout_elapsed(state.parser.sync_timeout().sync_timeout(), Instant::now())
                {
...
fn sync_timeout_elapsed(deadline: Option<Instant>, now: Instant) -> bool {
...
mod tests {
    ...
    use super::sync_timeout_elapsed;

    #[test]
    fn synchronized_update_timeout_depends_on_deadline() {
        let now = Instant::now();

        assert!(!sync_timeout_elapsed(None, now));
        assert!(!sync_timeout_elapsed(
            Some(now + Duration::from_millis(1)),
            now,
        ));
        assert!(sync_timeout_elapsed(Some(now), now));
        assert!(sync_timeout_elapsed(
            Some(now - Duration::from_millis(1)),
            now,
        ));
